...
//---------------------------------------------------------------------------------------------
ClassData * Loader::getClassData(char * className)
{
	/**
	 * psudoCode:
	 ...
	 * return classData;
	 */
	 char path[80]="/Khaled/eclipseWork/hello/";
	 ...
	 if(cdata == NULL)//Class not loaded
	 {
		...
		classFile=this->makeArrayInCPP(path);//*********************************************************************************DONT forget to uncomment
	 	...
	 	cdata = heap->createClassData(classFile); //*************************************************dont forget to modify it to take array of bytes or u1
